...
public class PlayerActivity extends AppCompatActivity implements android.view.View.OnClickListener{
    ...
    public void onClick(View v){
        if(!this.finish) {
            if (this.nextPlayer) {
                boolean nextTurn = playermove(v.getId(), 1);
                check();
                if (nextTurn) {
                    this.nextPlayer = false;
                }
            } else {
                boolean nextTurn = playermove(v.getId(), 2);
                check();
                if (nextTurn) {
    ...
    private void check (){
        if(f.sieg(1)){
            Toast.makeText(PlayerActivity.this, "Congratulation Player 1", Toast.LENGTH_SHORT).show();
            this.finish = true;
            findFour(1);
        }
        else if(f.sieg(2)){
            Toast.makeText(PlayerActivity.this, "Congratulation Player 2", Toast.LENGTH_SHORT).show();
            this.finish = true;
            findFour(2);
        }
        else if(f.full()){
            Toast.makeText(PlayerActivity.this, "Playfield full", Toast.LENGTH_SHORT).show();
            this.finish = true;
        }
    }

    private boolean playermove(int btnID, int player){
        Spielfeld foo = null;
        switch (btnID) {
            case R.id.btn0:
                foo = this.f.zug(player, 0);
                break;
            case R.id.btn1:
                foo = this.f.zug(player, 1);
                break;
            case R.id.btn2:
                foo = this.f.zug(player, 2);
                break;
            case R.id.btn3:
                foo = this.f.zug(player, 3);
                break;
            case R.id.btn4:
                foo = this.f.zug(player, 4);
                break;
            case R.id.btn5:
                foo = this.f.zug(player, 5);
                break;
            case R.id.btn6:
                foo = this.f.zug(player, 6);
                break;
        }
        if(foo == null){
            return false;
